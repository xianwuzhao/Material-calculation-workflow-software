...
def parse_multiline_string(filelines):
    """Parse semicolon-enclosed multiline string and return it."""
    global ciffilenumber
    line = filelines[ciffilenumber]
    assert line[0] == ';'
    lines = [line[1:].lstrip()]
    while True:
        ciffilenumber += 1
        line = filelines[ciffilenumber].strip()
        if line == ';':
            break
        lines.append(line)
    return '\n'.join(lines).strip()
...
def parse_singletag(filelines):
    ...
    if len(kv) == 1:
        ...
        while not line or line[0] == '#':
            ...
            line = filelines[ciffilenumber].strip()
        if line[0] == ';':
            value = parse_multiline_string(filelines)
        else:
            value = line
    else:
        key, value = kv
...
def parse_loop(filelines):
    ...
    while True:
        ...
        if line.startswith('#'):
            ciffilenumber += 1
            if ciffilenumber >= len(filelines):
                break
            else:
                line = filelines[ciffilenumber]
            continue
        if line.startswith(';'):
            t = [parse_multiline_string(filelines)]
        else:
            t = shlex.split(line)
        ...
        line = filelines[ciffilenumber].strip()
    if line:
        ciffilenumber = ciffilenumber - 1
        #unread_line(fileobj)
    return columns


def parse_items(filelines):
    ...
    while True:
        ...
        if not line or line.startswith('#'):
            ...
            key, value = parse_singletag(filelines)
            tags[key.lower()] = value
        elif lowerline.startswith('loop_'):
            tags.update(parse_loop(filelines))
        elif lowerline.startswith('data_'):
            ciffilenumber = ciffilenumber - 1
            #unread_line(fileobj)
            break
        elif line.startswith(';'):
            temp = parse_multiline_string(filelines)
        else:
            raise ValueError('Unexpected CIF file entry: "%s"'%line)
    return tags


def parse_block(filelines):
    """Parse a CIF data block and return a tuple with the block name
    and a dict with all tags."""
    global ciffilenumber
    line = filelines[ciffilenumber]
    assert line.startswith('data_')
    blockname = filelines[ciffilenumber].split('_', 1)[1].rstrip()
    tags = parse_items(filelines)
...
def parse_cif(fileobj):
    ...
    while True:
        ...
        if not line or line.startswith('#'):
            ciffilenumber += 1
            continue
        blocks.append(parse_block(filelines))
        ciffilenumber += 1
    return blocks
...
def tags2atoms(tags, store_tags=False, **kwargs):
    ...
    *store_tags* is true, the *info* attribute of the returned Atoms
    ...
    spacegroup = 1
    datafile = os.path.join(os.path.join(os.getcwd(),'Share'),'spacegroup.dat')
    if sitesym is not None:
        spacegroup = spacegroup_from_data(no=no, symbol=symbolHM,
                                          sitesym=sitesym,datafile=datafile)
    elif no is not None:
        spacegroup = no
